...
//! (plus per-line syntax highlighting) into plain owned structs that live happily inside Freya
//! `State`s and are cheap to clone into the builder tree. Colors are kept as raw `(u8, u8, u8)`
//! tuples so the UI can hand them straight to Freya's `Color::from`.

use crate::git::{ChangeKind, DiffSet, LineKind};
use crate::highlight::Highlighter;

/// One coloured piece of a code line, ready to become a styled `Span`.
